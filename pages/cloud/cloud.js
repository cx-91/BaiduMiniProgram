...
Page({
  ...
  onLoad: function () {
    ...
    this.resetAngle();
    swan.showToast({
      title: 'startGyroscope暂时不支持，请酌情处理',
      icon: 'none'
    });
    swan.onGyroscopeChange(res => {
      console.log(res);
      angleX += res.x / 30;
      angleY += res.y / 30;
      angleZ += res.z / 30;
  ...
  init: function () {
    ...
    var ctx = swan.createCanvasContext('cloud');
    tInt = setInterval(() => {
      ctx.translate(rpx2px(size / 2 + (750 - size) / 2), rpx2px(size / 2 + (750 - size) / 2));
      ctx.setFontSize(20); //ctx.fillText('center', -ctx.measureText('center').width / 2, 0)
      ...
      for (var i = 0; i < words.length / 2; i++) {
        ...
        words[i].y = y3;
        words[i].s = (words[i].z + size) / size;
        ctx.setFontSize(20 * words[i].s); //ctx.fillText(words[i].title, rpx2px(words[i].x) - ctx.measureText(words[i].title).width / 2, rpx2px(words[i].y))
